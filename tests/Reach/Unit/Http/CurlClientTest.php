...
namespace Reach\Tests\Unit\Http;
...
class CurlClientTest extends UnitTest {
    ...
    public function testPreemptiveAuthorization(): void {
        ...
        $apiUserPayload = \explode(' ', $apiuser);
        $apiUserPayload = \array_pop($apiUserPayload);
        $this->assertEquals('test-user', $apiUserPayload);
        $apiKeyPayload = \explode(' ', $apikey);
        $apiKeyPayload = \array_pop($apiKeyPayload);
        $this->assertEquals('test-password', $apiKeyPayload);
    }
     ...
     * @throws \Reach\Exceptions\EnvironmentException
     */
