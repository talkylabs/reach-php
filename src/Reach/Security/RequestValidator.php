...
/**
 * RequestValidator is a helper to validate that a request to a web server was actually made from Reach
 * EXAMPLE USAGE:
 * $validator = new RequestValidator('your auth token here');
 * $isFromReach = $validator->validate($_SERVER['HTTP_X_REACH_SIGNATURE'], 'https://your-example-url.com/api/route/', $_REQUEST);
 * $isFromReach // <- if this is true, the request did come from Reach, if not, it didn't
 */
class RequestValidator {

    /**
     * @access private
     * @var string The auth token to the Reach Account
     */
    private $authToken;
     ...
     * constructor
     * @access public
     * @param string $authToken the auth token of the Reach user's account
     ...
     * @param array $data the Reach parameters the request was made with
     * @return string
     */
    ...
    /**
     * The only method the client should be running...takes the Reach signature, their URL, and the Reach params and validates the signature
     *
